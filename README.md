...
| [Documentation](https://monarch-initiative.github.io/alliance-variant-allele-ingest) |

Koza transform of Alliance of Genome Resources variant/allele records
...
```bash
cd alliance-variant-allele-ingest
make install
make test
```
...
Download the data for the alliance_variant_allele_ingest transform:

```bash
poetry run alliance_variant_allele_ingest download
```

To run the Koza transform for alliance-variant-allele-ingest:

```bash
poetry run alliance_variant_allele_ingest transform
```

To see available options:

```bash
poetry run alliance_variant_allele_ingest download --help
# or
poetry run alliance_variant_allele_ingest transform --help
```
