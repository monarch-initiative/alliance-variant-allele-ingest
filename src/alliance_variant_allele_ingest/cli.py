"""Command line interface for alliance-variant-allele-ingest."""
import logging
...
def callback(version: bool = typer.Option(False, "--version", is_eager=True),
    ...
    """alliance-variant-allele-ingest CLI."""
    if version:
        from alliance_variant_allele_ingest import __version__
        typer.echo(f"alliance-variant-allele-ingest version: {__version__}")
        raise typer.Exit()


@app.command()
def download(force: bool = typer.Option(False, help="Force download of data, even if it exists")):
    """Download data for alliance-variant-allele-ingest."""
    typer.echo("Downloading data for alliance-variant-allele-ingest...")
...
def transform(
    ...
    """Run the Koza transform for alliance-variant-allele-ingest."""
    typer.echo("Transforming data for alliance-variant-allele-ingest...")
